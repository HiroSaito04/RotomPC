// rotompc-client\src\pages\Admin\UsersPage.jsx
...
function UsersPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (modal.id && form.password && form.password.length < 8) {
      return alert(
        "The updated character password must be at least 8 characters long.",
      );
    }
    ...
    try {
      ...
      alert(
        err.response?.data?.message ||
          "Failed to save user configuration updates.",
      );
    }
  };
  ...
  const toggleStatus = async (user) => {
    try {
      ...
      alert(
        err.response?.data?.message ||
          "Failed to update account status configuration",
      );
    }
  };
  ...
  const handleOpenDeleteConfirmation = (row) => {
    if (row._id === currentAdminId) {
      return alert(
        "Operation Prohibited: You cannot delete your own profile account from this control dashboard.",
      );
    }
    setUserToDelete(row);
  ...
  const handleConfirmDelete = async () => {
    ...
    if (!currentAdminId) {
      return alert(
        "Security Block: Your Admin Identification token is missing from local storage. Please re-login.",
      );
    }
    ...
    try {
      ...
      alert(
        err.response?.data?.message ||
          "Error permanently removing user profile registry.",
      );
    }
  };
  ...
  const columns = [
    ...
    { field: "email", headerName: "Email", flex: 1.2 },
    {
      ...
      renderCell: (p) => (
        <Typography
          sx={{ textTransform: "capitalize", fontSize: "0.875rem", mt: 1.5 }}
        >
          {p.value}
        </Typography>
      ),
      ...
      <Dialog open={modal.open} onClose={closeModal} fullWidth maxWidth="sm">
        <form onSubmit={handleSubmit}>
              ...
              <FormControlLabel
                control={
                  <Switch
                    ...
                    disabled={modal.id === currentAdminId}
                    onChange={(e) =>
          ...
          <DialogActions sx={{ p: 3 }}>
            <Button onClick={closeModal} sx={{ color: "#666" }}>
              Cancel
            </Button>
            <Button
              type="submit"
              variant="contained"
              sx={{ bgcolor: "#cc0000", px: 4 }}
            >
      ...
      <Dialog open={deleteDialogOpen} onClose={handleCloseDeleteDialog}>
        ...
        <DialogContent sx={{ mt: 2 }}>
          <DialogContentText sx={{ color: "#1a1a1a", fontWeight: "bold" }}>
            ...
            {userToDelete?.username})? This operation wipes their dataset and
            cannot be reversed.
          </DialogContentText>
        </DialogContent>
